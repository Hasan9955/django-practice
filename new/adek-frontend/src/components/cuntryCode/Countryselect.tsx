...
/** Full global list — ISO 3166-1 alpha-2 + E.164 dial codes */
export const COUNTRIES: Country[] = [
  ...
  { name: "Antigua & Barbuda", iso2: "AG", dialCode: "+1-268" },
  ...
  { name: "Saint Kitts & Nevis", iso2: "KN", dialCode: "+1-869" },
  { name: "Saint Lucia", iso2: "LC", dialCode: "+1-758" },
  { name: "Saint Vincent", iso2: "VC", dialCode: "+1-784" },
  { name: "Samoa", iso2: "WS", dialCode: "+685" },
  { name: "San Marino", iso2: "SM", dialCode: "+378" },
  { name: "São Tomé & Príncipe", iso2: "ST", dialCode: "+239" },
  ...
  { name: "Trinidad & Tobago", iso2: "TT", dialCode: "+1-868" },
...
/** Render a country flag emoji from ISO2 code */
  ...
  return (
    <div ref={containerRef} className="relative" style={{ minWidth: 130 }}>
      ...
      <button
        ...
        className="flex items-center gap-1.5 border border-gray-300 rounded-md px-3 py-[5px] bg-white hover:border-blue-500 transition-colors w-full h-[32px] text-sm"
        style={{ height: 32 }}
      >
        <span className="text-xl leading-none">{getFlagEmoji(selected.iso2)}</span>
        <span className="font-medium text-gray-700 flex-1 text-left">{selected.dialCode}</span>
        <DownOutlined
      ...
      {open && (
        <div
          className="absolute z-50 bg-white border border-gray-200 rounded-lg shadow-xl mt-1"
          style={{ width: 280, maxHeight: 340, display: "flex", flexDirection: "column" }}
        >
          {/* Search */}
          ...
          {/* List */}
          <div className="overflow-y-auto flex-1" style={{ maxHeight: 270 }}>
            {filtered.length === 0 ? (
              <div className="text-center text-gray-400 py-6 text-sm">No results</div>
            ) : (
              filtered.map((country) => (
                <button
                  ...
                  className={`flex items-center gap-2 w-full px-3 py-2 text-sm text-left hover:bg-blue-50 transition-colors ${
                    selected.iso2 === country.iso2 && selected.dialCode === country.dialCode
                      ? "bg-blue-50 font-medium"
                      : ""
                  }`}
                >
                  <span className="text-xl leading-none w-7 shrink-0">{getFlagEmoji(country.iso2)}</span>
...
// ─── Phone validation helper using google-libphonenumber ──────────────────────

/**
 * Validates a local phone number against the selected country.
 * @param localNumber  - digits only, no country code (e.g. "7911123456")
 * @param dialCode     - e.g. "+44"
 * @returns true if valid
 */
export function validatePhoneNumber(localNumber: string, dialCode: string): boolean {
  try {
    const country = COUNTRIES.find((c) => c.dialCode === dialCode);
    const regionCode = country?.iso2 ?? "ZZ";
    const parsed = phoneUtil.parseAndKeepRawInput(localNumber, regionCode);

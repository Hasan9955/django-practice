...
import { sendOrderConfirmationFlow } from "../utlits/orderConfirmationFlow";
...
const handleWebHook = catchAsync(async (req: any, res: any) => {
  ...
  switch (event.type) {
    ...
    case "checkout.session.completed":
      ...
      if (
        completedSession.metadata?.orderIds &&
        completedSession.metadata?.paymentId
      ) {
        const orderIdsArray = completedSession.metadata.orderIds.split(",");

        await prisma.payment.update({
          where: {
            id: completedSession.metadata.paymentId,
          },
          data: {
            status: "Paid",
          },
    ...
    if (orders.length > 0) {
      await sendOrderConfirmationFlow(orders);
    }

...
const updateConversationList = async (
  type: "conversation" | "ticket" | "refund",
  user1Id: string,
  user2Id: string,
  entityId: string,
  lastMessage: any
  ...
  await Promise.all([
    redis.zadd(`${listKey}:${user1Id}`, timestamp, entityId),
    redis.zadd(`${listKey}:${user2Id}`, timestamp, entityId),
    redis.zremrangebyrank(`${listKey}:${user1Id}`, 0, -MAX_CONVERSATIONS - 1),
    redis.zremrangebyrank(`${listKey}:${user2Id}`, 0, -MAX_CONVERSATIONS - 1),
    redis.hset(
      `${detailKey}:${entityId}`,
      "lastMessage",
      messagePreview,
...
const getConversationListFromRedis = async (
  ...
  const listKey = `${type}:list:${userId}`;
  const detailKeyPrefix = `${type}:details`;

  const entityIds = await redis.zrevrange(listKey, start, end);

  if (!entityIds.length) return null;

  const items = await Promise.all(
    entityIds.map(async (eid: any) => {
      const details = await redis.hgetall(`${detailKeyPrefix}:${eid}`);
  ...
  return {
    ...
    meta: {
      page,
      limit,
      total: await redis.zcard(listKey),
    },
  };
};

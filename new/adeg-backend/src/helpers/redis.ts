...
const ticketMessagePersistenceQueue = new Queue("ticketMessageQueue", {
  connection: redis,
});
const refundMessagePersistenceQueue = new Queue("refundMessageQueue", {
  connection: redis,
});
        ...
        parsed.map((m) =>
          prisma.privateMessage.upsert({
            ...
            create: {
              ...
              messageType: m.chatType || 'PRIVATEMESSAGE'
            },
          })
        )
      );
...
const ticketMessageWorker = new Worker(
  "ticketMessageQueue",
  async (job) => {
    const { ticketId } = job.data;
    
    const redisKey = `ticket:messages:${ticketId}`;
    const backupKey = `ticket:messages:${ticketId}`;
    ...
    if (!rawMessagesWithScores.length) {
      return `No ticket messages to persist for ${ticketId}`;
    }
    ...
    try {
      ...
      await Promise.all([redis.del(redisKey), redis.del(backupKey)]);
      return `✅ Persisted ${parsed.length} ticket messages for ${ticketId}`;
    } catch (error: any) {
      return `❌ DB error (ticket): ${error.message || error}`;
    }
  },
  { connection: redis }
);

const refundMessageWorker = new Worker(
  "refundMessageQueue",
  async (job) => {
    const { refundId } = job.data;

    const redisKey = `refund:messages:${refundId}`;
    const backupKey = `refund:messages:${refundId}`;
    ...
    if (!rawMessagesWithScores.length) {
      return `No refund messages to persist for ${refundId}`;
    }
    ...
    try {
      ...
      await Promise.all([redis.del(redisKey), redis.del(backupKey)]);
      return `✅ Persisted ${parsed.length} refund messages for ${refundId}`;
    } catch (error: any) {
      return `❌ DB error (refund): ${error.message || error}`;
    }
  },
  { connection: redis }
);
...
const fileUploadWorker = new Worker(
  ...
  async (job) => {
    ...
    console.log(`📦 Processing file-upload job for post: ${postId}`);
    ...
    const uploadedUrls = await Promise.all(
      files.map(async (file: any) => {
        try {
          ...
          return uploaded.Location || uploaded; // depending on your uploader’s return
        } catch (err) {
...
fileUploadWorker.on("failed", (job, err) => {
  console.error(`💥 File-upload job failed: ${job?.id}`, err);
});
...
export async function cleanQueues() {
  await Promise.all([
    ...
    ticketMessagePersistenceQueue.clean(0, 1000, "completed"),
    ticketMessagePersistenceQueue.clean(0, 1000, "failed"),
    ticketMessagePersistenceQueue.clean(0, 1000, "delayed"),
    ticketMessagePersistenceQueue.clean(0, 1000, "wait"),

    refundMessagePersistenceQueue.clean(0, 1000, "completed"),
    refundMessagePersistenceQueue.clean(0, 1000, "failed"),
    refundMessagePersistenceQueue.clean(0, 1000, "delayed"),
    refundMessagePersistenceQueue.clean(0, 1000, "wait"),
...
ticketMessageWorker.on("failed", handleJobFailure);
refundMessageWorker.on("failed", handleJobFailure);
...
process.on("SIGINT", async () => {
  ...
  await ticketMessageWorker.close();
  await refundMessageWorker.close();
